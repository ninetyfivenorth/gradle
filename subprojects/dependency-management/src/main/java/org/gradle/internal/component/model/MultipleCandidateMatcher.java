...
class MultipleCandidateMatcher<T extends HasAttributes> {
    ...
    private Attribute<?> getAttribute(int a) {
        return a < requestedAttributes.length ? requestedAttributes[a] : extraAttributes[a - requestedAttributes.length];
    }
    ...
    private Object getCandidateValue(int c, int a) {
        if (a < requestedAttributes.length) {
            ...
            AttributeValue attributeValue = candidateAttributeSets[c].findEntry(extraAttribute);
            return attributeValue.isPresent() ? attributeValue.get() : null;
        }
    }
